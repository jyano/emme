w.ratios=function(){
var w=this
	w.wRatio = w.Ww = w.gameW / w.worldW
	w.hRatio = w.Hh = w.gameH / w.worldH
}
w.multiples=function(){
var w=this
	w.wMultiple = 1 / w.wRatio
	w.hMultiple = 1 / w.hRatio
}
w.halves=function(){
var w=this
	w.halfW = w.hW = w.W / 2
	w.halfH = w.hH = w.H / 2
}
w.scales=function(){var w=this
	w.minScale = w.minZoom = w.mS = 1 / lower(w.wMultiple, w.hMultiple);
	w._zoom = w._scale = 1;
	w.z = w._zoom
}
w.setupDimensions = function () {var w = this

	w.worldW = w.w = o.wW

	w.worldH = w.h = o.wH

	w.gameW = w.W = o.W
	w.gameH = w.H = o.H
	w.ratios()
	w.halves()
	return w
}
w.setupRatios=function(){var w=this
	w.ratios()
	w.multiples()
	
	w.scales()

}
w.scrollDimensions = function (o) {
	var w = this
	w.worldW = w.w = o.wW
	w.worldH = w.h = o.wH
	w.gameW = w.W = o.W
	w.gameH = w.H = o.H
	
	w.setupRatios()
	
	w.halfW = w.hW = w.W / 2
	w.halfH = w.hH = w.H / 2
	return w
}
w.dimensions = function () {
	var w = this
	//half width/height nobiggie..
	w.W2 = w.HW = w.hW = w.W / 2;
	w.H2 = w.HH = w.hH = w.H / 2;
	w.hH = w.h / w.H;
	w.Ww = w.W / w.w;
	w.wW = w.w / w.W;
	w.Hh = w.H / w.h;
	w.mZ = w.hH > w.wW ? w.hH : w.wW;
	w.mS = w.Ww > w.Hh ? w.Ww : w.Hh;
	//scale
	w.z = w.SCALE = 1
}
bx.initialDimensions = function (o) {
	o.W = o.W || 1200
	o.H = o.H || 600
	o.wW = o.wW || o.W
	o.wH = o.wH || o.H
	o.w = o.w == 0 ?
			0 : o.w ? o.w : ['o', o.wW, o.wH]
	o.g = N(o.g) ? V(0, o.g) : o.g ? V(o.g) : V(0, 300)
	return o
}


function yadaYadaYada() {
	var o = N(H) ? {W: W, H: H, wW: wW, wH: wH} :
			N(W) ? {g: W} : A(W) ? _.x(H, {W: W[0], H: W[1], wW: W[2], wH: W[3]}) :
					O(W) ? W : {}


	o=bx.initialDimensions(o)
	w = b2d.world(o.g);
	cjs.watchKeys()
	w.o = o
	if (o.z != false) {z()}
}

function yadaSetupYada() {
	w.gx(o).setBg(o).dd(o).db().startListening().
			wMouse().wMouseJ(o).walls(o).u(o)
}

w.widthHeight = function () {
	var w = this
	o.W = N(o.W, 1200)
	o.H = N(o.H, 600)
	w.W = o.W
	w.H = o.H
	w.w = N(o.wW, w.W);
	w.h = N(o.wH, w.H);
	w.W2 = w.HW = w.hW = w.W / 2;
	w.H2 = w.HH = w.hH = w.H / 2;
	w.hH = w.h / w.H;
	w.Ww = w.W / w.w;
	w.wW = w.w / w.W;
	w.Hh = w.H / w.h;
	w.mZ = w.hH > w.wW ? w.hH : w.wW;
	w.mS = w.Ww > w.Hh ? w.Ww : w.Hh;
	//scale
	w.z = w.SCALE = 1
	return w
}




function room() {
	w.BG = w.I = $St('z', w.W, w.H, 0, 0)
	w.st = w.s = $St('X', w.W, w.H, 0, 0)//.aC(0)
	w.cv = w.canvas = w.st.canvas;
	w.$cv = w.can = $(w.canvas);
	w.cx = w.ctx = w.can.ctx('2d')
	w.bg = w.st.ct();
	w.gx = w.g = w.st.ct();
	w.fg = w.st.ct()
	w.FG = w.i = $St('X', w.W, w.H, 0, 0)
	if (o.i) {
		w.s.bm(o.i)
	}
	w.lG($r())
	if (o.aC == 1) {
		//w.i.aC(0)
		// if(g.O){; return w}
		// return g.u? w.i.aC(!w.i.aC()): g.s? w.i.aC(w):
	}
	w.walls(_w)
}
w.zoomWorldSetup = function () {
	var w = this
	w.setupDimensions()
	w.setupRatios()
	return w
}
$zoomWorld = function (W, H, wW, wH) {
	cjs.Ticker.removeAllEventListeners()
	w.show(function () {
	})//not working with scroll
	return w.zoomWorldSetup()
}